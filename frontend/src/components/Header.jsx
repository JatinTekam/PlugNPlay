...
function Header({handleDarkMode,darkMode}) {
  ...
  return (
    <div className="w-screen h-20 pt-4">
      ...
      <div className={`mobile-sideMenu w-95 z-11 h-[108rem] pt-30 text-center ${mobileMenu ? "block" : "hidden"}  backdrop-blur-md top-15 absolute right-0`}>
          <a href="" className={`text-2xl ${darkMode ? "text-black": "text-white"}`}>Show Template's</a>
      </div>
    </div>
  );
}
